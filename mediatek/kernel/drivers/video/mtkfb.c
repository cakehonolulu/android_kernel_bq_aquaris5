...
//    }
	{
        u32  pbuf = 0;
        void *buff, *buff_align;
        unsigned int buff_size, fb_size;
        ...
        else 
        {
             ...
             fb_size = MTK_FB_XRES*MTK_FB_YRES*(MTK_FB_BPP/8);
             buff_size = fb_size + 32;
    
             buff = vmalloc(buff_size);
             printk("[MTKFB_CAPTURE_FB] buffer size = %d, buf_addr = 0x%x\n", buff_size, buff);
             if(!buff)
             {
             ...
             {
                  buff_align = (void *)((unsigned long)buff + 32 - ((unsigned long)buff & 0x1F)); //for M4U 32-byte alignment
                   printk("[MTKFB_CAPTURE_FB] buff_align_addr = 0x%x\n", buff_align);
                   mtkfb_capture_framebuffer(info, buff_align);
                   if(copy_to_user((void  __user *)pbuf, buff_align, fb_size))
                   {
